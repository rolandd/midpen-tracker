...
>
	...
	<h3 class="font-semibold text-[var(--color-text)] mb-2 {variant === 'compact' ? 'text-base' : 'text-lg'}">
		{title}
	</h3>

	{#if description}
		<p class="text-[var(--color-text-muted)] max-w-sm mb-6 leading-relaxed {variant === 'compact' ? 'text-xs mb-3' : 'text-sm'}">
			{description}
		</p>
	{/if}
