...
fn benchmark_find_intersections(c: &mut Criterion) {
    ...
    let real_line = polyline::decode_polyline(polyline_str, 5)
    ...
    let shifted_line_coords: Vec<_> = real_line.0.iter()
        .map(|c| (c.x + 5.0, c.y))
        .collect();
    ...
    group.bench_function("real_activity_intersects", |b| {
        b.iter(|| {
            service.find_intersections(black_box(&real_line))
        })
    });

    group.bench_function("shifted_activity_far_away", |b| {
        b.iter(|| {
            service.find_intersections(black_box(&shifted_line))
        })
    });
